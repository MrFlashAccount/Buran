...
## WorkerTask and adapter identity

`worker_tasks.head` is the durable inner lifecycle for active implementation, fix, and review workers. Active dispatch records may include `adapter_id`, `adapter_task_id`, `adapter_status`, `heartbeat_at`, and `status_summary_ref`; these fields are public-safe coordination metadata, not raw worker transcripts. A later `/buran run` must reattach or poll an active adapter task when possible. If an active adapter task exists but the injected adapter cannot reattach or poll, the runner records/returns `UNKNOWN` wait state rather than spawning a duplicate worker.

Implementation and fix workers accept terminal completion statuses through core WorkerTask decisions only for statuses accepted by the core completion contract (`COMPLETED`, `FAILED`, `BLOCKED`, and `CANCELLED`). Waiting dispatch statuses (`PENDING`, `UNKNOWN`, `STALE`) are not WorkerCompletion statuses. A recorded waiting result remains reusable even when `adapter_task_id` is absent; resume must wait safely instead of spawning another worker. When adapter identity is present, `adapter_id`, `adapter_task_id`, `adapter_status`, `heartbeat_at`, and `status_summary_ref` are projected onto the active WorkerTask and operator status so the next runner pass can reattach/poll. Review attempts are recorded with purpose `review_attempt`; internal-review `FAIL` and `BLOCKED` are mapped to accepted terminal WorkerCompletion statuses before any outer transition so the review WorkerTask decision is enforceable by the same core guard.
...
Implementation-dispatch `problem` uses a small safe shape: `code` plus generic `message`; extra raw adapter fields are intentionally not copied into immutable artifacts or public runner reports. `PENDING`, `UNKNOWN`, and `STALE` keep the run in `running` and preserve wait/reattach state; `CANCELLED` is a non-success terminal dispatch result that does not advance to verification. `BLOCKED` keeps the run in `running`; `FAILED` transitions `running -> failed_execution`; only `COMPLETED` with durable evidence and an accepted WorkerTask completion decision transitions to `verification`.

Fix-attempt dispatch uses the same sanitizer and durable-evidence rules, but from `fix_loop`. `COMPLETED` transitions back to `verification`, which creates a fresh execution epoch and resets verification/internal-review gate heads. `PENDING`, `UNKNOWN`, and `STALE` are waiting statuses: they preserve the active `WorkerTask`/adapter task identity for reattach or poll and do not record WorkerCompletion, consume a completed fix-attempt budget slot, or spawn a duplicate worker on resume. `BLOCKED`, `FAILED`, and `CANCELLED` are terminal dispatch outcomes for the attempt and keep the run in `fix_loop` or a terminal blocked/failed state according to the runner stage. The current local bound is two completed fix attempts; once exhausted, the next fix-loop pass transitions to `blocked_needs_human`.
...
- Artifact record: write artifact content, append `artifact.recorded`, update `run.json` artifact head/`last_sequence`, then return. Implementation-dispatch intent/result artifacts are recorded as `gate_name: implementation_dispatch` with `recorded_from_state: running`; resume reuses a current-epoch terminal result artifact with matching dispatch intent and hash instead of invoking the adapter again, while waiting results (`PENDING`/`UNKNOWN`/`STALE`) preserve adapter task identity and reattach/poll instead of spawning duplicates. Fix-loop intent/result artifacts are recorded as `gate_name: fix_attempt` with `recorded_from_state: fix_loop`; resume reuses a current-epoch terminal result artifact whose hash, attempt number, custom intent path, and dispatch intent match the current fix-attempt intent, while waiting results preserve the active fix `WorkerTask` for reattach/poll and do not count as completed attempts.
...
`WorkerTask` identity is derived from `run_id`, `task_id`, purpose, role, epoch, attempt, and authority. Every task head/history entry and worker-task event payload carries an explicit `role`. Current purpose-to-role mapping is `implementation_dispatch` -> `implementer` and `fix_attempt` -> `fixer`; `reviewer` is reserved for future review-worker behavior and is schema-valid but has no runner behavior in this slice. `WorkerCompletion` stores only sanitized status, role, evidence, and artifact refs. `CompletionDecision` is registry/core-owned and may accept, defer, mark late/duplicate/conflict/unknown/unauthorized, or quarantine. Recovery replays these typed events and quarantines mismatched task ids, conflicting idempotency payloads, or completion events without a current task head.
...
Worker task events are first-class non-transition events in `events.jsonl`: `worker_task.created`, `worker_task.dispatch_recorded`, `worker_task.completion_received`, `worker_task.completion_decided`, `worker_task.overdue_recorded`, and `worker_task.quarantined`. Payloads carry task identity, role, epoch, attempt, authority, safe artifact refs, status/decision, completion status, and idempotency keys. Raw worker content is not part of the public schema; summaries and reports use sanitized refs only.

A completion cannot move the outer run until a durable `worker_task.completion_decided` event records an `accepted` `CompletionDecision`. Issue #16 terminology maps to durable events as follows: completed/failed = `worker_task.completion_decided` with `decision=accepted` plus `completion_status=COMPLETED|FAILED`; timed-out = `worker_task.overdue_recorded`; late/conflict = `worker_task.completion_decided` with `decision=late|conflict`. Duplicate idempotent events are `noop`; conflicting idempotency payloads fail instead of repairing into accepted truth.
