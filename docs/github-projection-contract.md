...
| `handoff_ready` | Create/update the configured `handoff_target`, or in local fake mode record the deterministic handoff artifact that would drive creation/update. In the current GitHub profile this can mean PR creation/update; it is not proof that a real remote write already happened. |
...
- explicit handoff identity. In the current GitHub profile this includes the canonical `scm_target.base_branch` and `scm_target.intended_branch` for review-target projection.
